...
movies_df[["Participant Name", "Title of Thing", "Genre"]]# note the double square brackets
```
...
#| echo: true
#| eval: true
movies_df[movies_df.columns[0:3]] #This will return the first three columns of our dataframe
```
...
# Slicing our Dataframe - `location` slicing {.smaller}
 ...
 - Essentially, we're passing 2 slices (seperated by a comma) to the `loc` method.
...
# Booleans {.smaller}
 ...
 - For example, if we wanted to filter our data to only show the rows where the `Genre` column is equal to `Sci-Fi`, we can use a boolean operation.
...
for i in movies_df.columns:
    movies_df[i].rename(columns ={i:i.lower()}, inplace = True)
```
...
#| echo: true
#| eval: true
df.columns = [i.replace(' ', '_') for i in df.columns]# isn't that awesome!!
```
...
`[do_thing(placeholder) for placeholder in collection]`
 ...
 - We can also call `methods` instea of `functions`
