...
Now we've talked about how python handles different types of data, we're going to start by looking at the (basic) things we can do with that data. 
 ...
 - We'll start with `indexing` and `slicing` strings
 - Then we'll look at `methods` and `functions` (again using strings as an example)
 - Then we'll take a look at `indexing` and `slicing` lists
 - Then we'll take a look at `loops`
 - Then we'll look at `conditionals` and introduce a new datatype, the `boolean`
