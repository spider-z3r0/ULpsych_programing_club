...
- We're going to start out looking at `strs` which is just the `data type` for alphanumeric characters (letters and numbers) or human language in python. 
...
```
![More pictures behind the slides!](./images/python_double_quotes_good.jpg)

 - Now you know how to write the names of the authors of this material ("O'Brien", "O'Malley" & 'Donovan')
 - But `single and double quotes` only work for quite short `strs` like single names or a short sentence. 
...
```
![This will throw an error](./images/python_trip_quotes_good.jpg)
